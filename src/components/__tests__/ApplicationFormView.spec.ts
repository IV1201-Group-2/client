...
describe("Applicant Registration View", () => {
  ...
  describe("Available Through", () => {
    ...
    describe("Add button", () => {
      ...
      it(
        Action.IsNotDisabled + " when the start and end date are set to yesterday (both start and end are in the past)",
        async () => {
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.StartDate))
            .setValue(yesterday.toISOString().substring(0, 10));
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.EndDate))
            .setValue(yesterday.toISOString().substring(0, 10));
          expect(addButtonEl!.attributes("disabled")).not.toBeUndefined();
        }
      );
      ...
      it(
        Action.IsNotDisabled + " when the start date is set to today and end date to tomorrow (start comes before end)",
        async () => {
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.EndDate))
            .setValue(tomorrow.toISOString().substring(0, 10));
          expect(addButtonEl!.attributes("disabled")).toBeUndefined();
        }
      );

      it(
        Action.IsDisabled + " when the start date is set to yesterday and the end date to today (start is in the past)",
        async () => {
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.StartDate))
            .setValue(yesterday.toISOString().substring(0, 10));
      ...
      it(
        Action.IsDisabled + " when start date is set to today end date to yesterday (ends comes before start)",
        async () => {
      ...
      it(
        Action.IsDisabled +
          " when start and end date is set to today, after having added availability between today and tomorrow (conflict with start)",
        async () => {
      ...
      it(
        Action.IsDisabled +
          " when the start and end date when start and end date is set to tomorrow, after having added availability between today and tomorrow (conflict with end)",
        async () => {
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.EndDate))
            .setValue(tomorrow.toISOString().substring(0, 10));
      ...
      it(
        Action.IsDisabled +
          " when availability has been added for tomorrow and the user attempts to add availability between today and the day after tomorrow (period contains added period)",
        async () => {
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.StartDate))
            .setValue(tomorrow.toISOString().substring(0, 10));
          ...
          await wrapper!
            .findComponent(getSelector(ApplicationTestId.EndDate))
            .setValue(dayAfterTomorrow.toISOString().substring(0, 10));
          expect(addButtonEl!.attributes("disabled")).not.toBeUndefined();
        }
      );
    });
