...
<template>
  <v-sheet width="300" class="mx-auto">
    <v-form v-model="isFormValid" @submit.prevent>
      ...
      <v-text-field
        :data-test="RegistrationTestId.FirstName"
        v-model="firstName"
        :rules="firstNameRules"
        :label="$t(fieldsPath + 'first-name')"
      />
      <v-text-field
        :data-test="RegistrationTestId.LastName"
        v-model="lastName"
        :rules="lastNameRules"
        :label="$t(fieldsPath + 'last-name')"
      />
      <v-text-field
        :data-test="RegistrationTestId.Email"
        v-model="email"
        :rules="emailRules"
        :label="$t(fieldsPath + 'email')"
      />
      <v-text-field
        :data-test="RegistrationTestId.PersonNumber"
        v-model="personNumber"
        :rules="personNumberRules"
        :label="$t(fieldsPath + 'person-number')"
      />
      <v-text-field
        :data-test="RegistrationTestId.Username"
        v-model="username"
        :rules="usernameRules"
        :label="$t(fieldsPath + 'username')"
      />
      <v-text-field
        :data-test="RegistrationTestId.Password"
        v-model="password"
        :rules="passwordRules"
        type="password"
        :label="$t(fieldsPath + 'password')"
      />
      <v-btn
